        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($row = $result->fetch_assoc()) {
                if (password_verify($mot_de_passe, $row['mot_de_passe'])) {
                    ...
                    echo "<script>alert('Connexion réussie! Bienvenue " . $row['nom'] . "'); window.location.href='index.php';</script>";
                    exit;
                } else {
                    echo "<script>alert('Mot de passe incorrect.');</script>";
                }
            } else {
                echo "<script>alert('Aucun utilisateur trouvé avec cet email.');window.location.href='login.html'</script>";
            }
